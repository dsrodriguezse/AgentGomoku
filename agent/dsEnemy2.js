class MinimaxAgentDSEnemy2 extends Agent {
    constructor() {
        ...
        this.maxDepth = 50; // Profundidad máxima para explorar el árbol de decisiones.
    ...
    evaluateBoard(board, color) {
        ...
        const scores = {
            ...
            closeEnemy1: Math.pow(50000, this.evaluateLinesEnemy(board, opponentColor, 3) + this.evaluateLinesEnemy(board, opponentColor, 4)),
            controlEnemy1: this.evaluateEnemyControl(board, color)*1000
        };
    ...
    evaluateEnemyControl(board, color) {
        const opponentColor = color === 'W' ? 'B' : 'W';
        let score = 0;
        const size = board.length;
                ...
                if (board[y][x] === opponentColor) {
                    ...
                    for (let dy = -1; dy <= 1; dy++) {
                        for (let dx = -1; dx <= 1; dx++) {
                            ...
                            const newX = x + dx;
                            const newY = y + dy;
                            if (newX >= 0 && newX < size && newY >= 0 && newY < size && board[newY][newX] === color) {
                                score += 1000; // Asignar puntaje por fichas adyacentes al enemigo
                            }
                        }
                    }
                }
